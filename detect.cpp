...
Mat preprocess(Mat gray)
{
    ...
    Mat element2 = getStructuringElement(MORPH_RECT, Size(40, 4));
...
Mat detect::det(Mat img)
{
    ...
    //4.用绿线画出这些找到的轮廓
    // for_each (RotatedRect rect in rects)
    for (vector<RotatedRect>::iterator itor = rects.begin(); itor != rects.end(); ++itor)
    {
        Point2f P[4];
        itor->points(P);
        for (int j = 0; j <= 3; j++)
